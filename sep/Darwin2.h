...
typedef enum{
    ...
    BUTTON_RSB = 14,
    BUTTON_LSB = 15,
}DW_CONTROLLER_BUTTON;

typedef struct{
    double x,y;
}DW_CONTROLLER_STICK_VALUE;

typedef struct{
    bool buttonsDown[16];
    bool buttonsPressed[16];
    DW_CONTROLLER_STICK_VALUE leftStick;
    DW_CONTROLLER_STICK_VALUE rightStick;
}DW_CONTROLLER;
...
bool DW_IsControllerButtonDown(DW_CONTROLLER_BUTTON button);
bool DW_IsControllerButtonPressed(DW_CONTROLLER_BUTTON button);

float DW_GetControllerLeftStickX();
float DW_GetControllerLeftStickY();
...
static DW_CONTROLLER currentControllerState = {0};
static DW_CONTROLLER previousControllerState = {0};
...
- (void)controllerDidDisconnect:(NSNotification *)notification {
    GCController *controller = notification.object;
    NSLog(@"Controller disconnected: %@", controller);
}
...
void processControllerInput() {
    ...
    for (GCController *controller in controllers) {
        if (controller.extendedGamepad) {
            ...
            currentControllerState.buttonsDown[BUTTON_A] = extendedGamepad.buttonA.isPressed;
            currentControllerState.buttonsDown[BUTTON_X] = extendedGamepad.buttonX.isPressed;
            currentControllerState.buttonsDown[BUTTON_Y] = extendedGamepad.buttonY.isPressed;
            currentControllerState.buttonsDown[BUTTON_B] = extendedGamepad.buttonB.isPressed;
            currentControllerState.buttonsDown[BUTTON_START] = extendedGamepad.buttonMenu.isPressed;
            currentControllerState.buttonsDown[BUTTON_SELECT] = extendedGamepad.buttonOptions.isPressed;
            currentControllerState.buttonsDown[BUTTON_DPAD_UP] = extendedGamepad.dpad.up.isPressed;
            currentControllerState.buttonsDown[BUTTON_DPAD_DOWN] = extendedGamepad.dpad.down.isPressed;
            currentControllerState.buttonsDown[BUTTON_DPAD_LEFT] = extendedGamepad.dpad.left.isPressed;
            currentControllerState.buttonsDown[BUTTON_DPAD_RIGHT] = extendedGamepad.dpad.right.isPressed;

            currentControllerState.buttonsDown[BUTTON_RT] = extendedGamepad.rightTrigger.isPressed;
            currentControllerState.buttonsDown[BUTTON_LT] = extendedGamepad.leftTrigger.isPressed;
            currentControllerState.buttonsDown[BUTTON_RB] = extendedGamepad.rightShoulder.isPressed;
            currentControllerState.buttonsDown[BUTTON_LB] = extendedGamepad.leftShoulder.isPressed;
            currentControllerState.buttonsDown[BUTTON_RSB] = extendedGamepad.leftThumbstickButton.isPressed;
            currentControllerState.buttonsDown[BUTTON_LSB] = extendedGamepad.rightThumbstickButton.isPressed;


            // Update stick values
            currentControllerState.leftStick.x = extendedGamepad.leftThumbstick.xAxis.value;
            currentControllerState.leftStick.y = extendedGamepad.leftThumbstick.yAxis.value;
            currentControllerState.rightStick.x = extendedGamepad.rightThumbstick.xAxis.value;
            currentControllerState.rightStick.y = extendedGamepad.rightThumbstick.yAxis.value;

            // Determine which buttons were pressed (state changed from not pressed to pressed)
            // Reset button press detection
            for (int i = 0; i < 16; ++i) {
                // Update buttonsPressed to reflect new presses
                if (currentControllerState.buttonsDown[i] && !previousControllerState.buttonsDown[i]) {
                    currentControllerState.buttonsPressed[i] = true;
                    NSLog(@"Button %d was pressed", i);
                } else {
                    currentControllerState.buttonsPressed[i] = false;
                }
            }
    ...
    HRESULT InitDirectInput(HINSTANCE hInst) {
        ...
        hr = IDirectInput2_CreateDevice(di,&GUID_Joystick, &diJoystick, NULL);
        if (FAILED(hr)) {
            return hr;
        }

        // Set data format
        hr = IDirectInputDevice2_SetDataFormat(diJoystick,&c_dfDIJoystick2);
        if (FAILED(hr)) {
            return hr;
        }

        // Set cooperative level
        hr = IDirectInputDevice2_SetCooperativeLevel(diJoystick,GetDesktopWindow(), DISCL_FOREGROUND | DISCL_EXCLUSIVE);
        if (FAILED(hr)) {
            return hr;
        }

        // Acquire the joystick
        hr = IDirectInputDevice2_Acquire(diJoystick);
        if (FAILED(hr)) {
            return hr;
        }
    ...
    void ProcessControllerInput() {
        HRESULT hr = IDirectInputDevice2_Poll(diJoystick);
        if (FAILED(hr)) {
            // Re-acquire the device if needed
            IDirectInputDevice_Acquire(diJoystick);
            IDirectInputDevice2_Poll(diJoystick);
        }

        hr = IDirectInputDevice_GetDeviceState(diJoystick,sizeof(DIJOYSTATE2), &jsState);
        if (SUCCEEDED(hr)) {
            // Update button states
            currentControllerState.buttonsDown[BUTTON_A] = (jsState.rgbButtons[0] & 0x80) != 0;  // Example for button 0
            currentControllerState.buttonsDown[BUTTON_B] = (jsState.rgbButtons[1] & 0x80) != 0;  // Example for button 1
            currentControllerState.buttonsDown[BUTTON_X] = (jsState.rgbButtons[2] & 0x80) != 0;  // Example for button 2
            currentControllerState.buttonsDown[BUTTON_Y] = (jsState.rgbButtons[3] & 0x80) != 0;  // Example for button 3
            // Map other buttons accordingly

            // Update stick values
            currentControllerState.leftStick.x = jsState.lX / 32768.0;
            currentControllerState.leftStick.y = jsState.lY / 32768.0;
            currentControllerState.rightStick.x = jsState.lRx / 32768.0;
            currentControllerState.rightStick.y = jsState.lRy / 32768.0;
        }
        ...
        // Log continuous button presses
        for (int i = 0; i < 14; ++i) {
            if (currentControllerState.buttonsDown[i]) {
            }
        }
...
void DW_WINDOW_New(int width,int height,const char* title){
    ...
    // Controller
    // if (FAILED(InitDirectInput(GetModuleHandle(NULL)))) {
    //     MessageBox(NULL, "DirectInput Initialization Failed", "Error", MB_OK | MB_ICONERROR);
    //     return;
    // }
...
bool DW_IsRunning() {
    ...
    static double fpsRefreshRate = 5.0; // Update FPS every second
...
bool DW_IsControllerButtonDown(DW_CONTROLLER_BUTTON button){
    return currentControllerState.buttonsDown[button];
}
bool DW_IsControllerButtonPressed(DW_CONTROLLER_BUTTON button){
    return currentControllerState.buttonsPressed[button];
}

float DW_GetControllerLeftStickX(){
    return currentControllerState.leftStick.x;
}
float DW_GetControllerLeftStickY(){
    return currentControllerState.leftStick.y;
}
float DW_GetControllerRightStickX(){
    return currentControllerState.rightStick.x;
}
float DW_GetControllerRightStickY(){
    return currentControllerState.rightStick.y;
}
